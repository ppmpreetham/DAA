...
vector<Point> convexHull(vector<Point>& pts){
    ...
    vector<Point> st={pts[0],pts[1],pts[2]}; // select 3 pts first
    for(int i=3;i<n;i++){ // for the rest of the pts
        while(st.size()>1 && orientation(st[st.size()-2], st.back(), pts[i]) != 2) // check for last 2 pts and current pt
            st.pop_back();
        st.push_back(pts[i]);
    }
    return st;
